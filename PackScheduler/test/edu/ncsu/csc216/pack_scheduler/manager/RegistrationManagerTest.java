...
public class RegistrationManagerTest {
	...
	/**
	 * Test the getCourseCatalog method
	 */
	@Test
	...
	/**
	 * Test the getStudentDirectory method
	 */
	@Test
	...
	/**
	 * Test the login(), logout(), and currentUser() functions
	 */
	@Test
	public void testLoginAndLogoutAndCurrentUser() {
		// Test login students
		manager.getStudentDirectory().loadStudentsFromFile("test-files/student_records.txt");

		Student test = manager.getStudentDirectory().getStudentById("efrost");
		assertEquals(test.getFirstName(), "Emerald");
		assertTrue(manager.login(test.getId(), "pw"));
		assertEquals(manager.getCurrentUser().getId(), "efrost");

		// Test login faculty
		manager.getFacultyDirectory().loadFacultyFromFile("test-files/faculty_records.txt");
		Faculty test1 = manager.getFacultyDirectory().getFacultyById("awitt");
		manager.logout();
		assertTrue(manager.login(test1.getId(), "pw"));
		assertEquals(manager.getCurrentUser().getId(), "awitt");

		// Test logging out student
		manager.logout();
		...
		// Test logging in with current user
		assertFalse(manager.login(test.getId(), "pw"));
		manager.logout();
		// Test invalid login
		assertFalse(manager.login("efrost", "password"));
		try {
			manager.login("robbert", "password");
		} catch (Exception e) {
	...
	/**
	 * Test the faculty functions of the registration manager
	 */
	@Test
	public void testFacultyFunctions() {
		CourseCatalog catalog = manager.getCourseCatalog();
		catalog.loadCoursesFromFile("test-files/course_records.txt");
		...
		Faculty awitt = manager.getFacultyDirectory().getFacultyById("awitt");
		manager.addFacultyToCourse(catalog.getCourseFromCatalog("CSC216", "001"), awitt);
		assertEquals("CSC216", awitt.getSchedule().getScheduledCourses()[0][0]);
		assertEquals("001", awitt.getSchedule().getScheduledCourses()[0][1]);

		manager.addFacultyToCourse(catalog.getCourseFromCatalog("CSC226", "001"), awitt);
		assertEquals("CSC226", awitt.getSchedule().getScheduledCourses()[1][0]);
		assertEquals("001", awitt.getSchedule().getScheduledCourses()[1][1]);

		manager.addFacultyToCourse(catalog.getCourseFromCatalog("CSC116", "003"), awitt);
		assertEquals("CSC116", awitt.getSchedule().getScheduledCourses()[2][0]);
		assertEquals("003", awitt.getSchedule().getScheduledCourses()[2][1]);

		manager.removeFacultyFromCourse(catalog.getCourseFromCatalog("CSC216", "001"), awitt);
		assertEquals("CSC226", awitt.getSchedule().getScheduledCourses()[0][0]);
		assertEquals("CSC116", awitt.getSchedule().getScheduledCourses()[1][0]);

		manager.resetFacultySchedule(awitt);
		assertEquals(0, awitt.getSchedule().getNumScheduledCourses());
	}
